document.getElementById("submit-datetime-btn").addEventListener("click", () => {
    ...
    const etimeBtn = document.getElementById("availableend");

    const dateStored = dateBtn.dataset.value;
    const stimeStored = stimeBtn.dataset.value;
    const etimeStored = etimeBtn.dataset.value;

    if (!dateStored || !stimeStored || !etimeStored) {
        console.warn("Date or start/end time not selected.");
        return;
    }
    ...
    // Parse "HHMM" → "HH:mm"
    const stimeFormatted = stimeStored.slice(0, 2) + ":" + stimeStored.slice(2);
    const etimeFormatted = etimeStored.slice(0, 2) + ":" + etimeStored.slice(2);

    const fromhr = dateISO + 'T' + stimeFormatted;
    const untilhr = dateISO + 'T' + etimeFormatted;
